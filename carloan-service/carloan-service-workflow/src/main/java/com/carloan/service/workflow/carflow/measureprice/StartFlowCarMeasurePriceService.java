...
public class StartFlowCarMeasurePriceService implements StartFlowBaseService {
    ...
    public Boolean startProcessByOrderId(String orderId)  {
        ...
        jbpm4BizTabExistParam.setBizType(CarFlowConst.CAR_TASKTYPE_DINGJIA);
        ...
        log.info(MessageFormat.format("********>>1.active start dingjia flow prepare,orderid:{0}",orderId));
        //发起流程
        ...
        if(ResponseResult.isSucess(result)&&result.getData()!=null){
            log.info(MessageFormat.format("2.active start dingjia flow sucess,orderid:{0}",orderId));
            ...
            jbpm4BizTabParam.setBizType(CarFlowConst.CAR_TASKTYPE_DINGJIA);//订单类型(定价)
            log.info(MessageFormat.format("3.save dingjia jbpm4_biz_tab prepare,orderid:{0},taskid:{1}",orderId,processId));
            Response response= jbpm4BizTabServiceFeign.saveJbpm4BizTab(jbpm4BizTabParam);
            if(Response.isSucess(response)){
                log.info(MessageFormat.format("4.startflow.dingjia.sucess,jbpm4BizTabParam:{0}",jbpm4BizTabParam.toString()));
                return true;
            }
            else{
