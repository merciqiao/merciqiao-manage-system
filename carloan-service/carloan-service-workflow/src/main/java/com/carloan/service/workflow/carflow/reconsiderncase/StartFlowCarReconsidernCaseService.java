...
@Service
@Slf4j
public class StartFlowCarReconsidernCaseService  implements StartFlowBaseService {
    @Autowired
    ...
    public Boolean startProcessByOrderId(String orderId) {
        ...
        jbpm4BizTabExistParam.setBizType(CarFlowConst.CAR_TASKTYPE_FUYI);
        ...
        String processKey = "carReconsidernCaseKey";//流程编码

        ActivitiInfoVO activitiInfoVO = new ActivitiInfoVO();
        activitiInfoVO.setOrderid(orderId);
        activitiInfoVO.setTransition(CarTransitionConst.CONST_FUYI_TURNDIR_START);
        activitiInfoVO.setAssignee("-1");
        activitiInfoVO.setProcessKey(processKey);

        log.info(MessageFormat.format("********>>1.active start fuyi flow prepare,orderid:{0}",orderId));
        //发起流程
        ...
        if (ResponseResult.isSucess(result)&& result.getData()!=null) {
            log.info(MessageFormat.format("2.active start fuyi flow sucess,orderid:{0}",orderId));
            ...
            jbpm4BizTabParam.setBizType(CarFlowConst.CAR_TASKTYPE_FUYI);//订单类型(定价)
            log.info(MessageFormat.format("3.save fuyi jbpm4_biz_tab prepare,orderid:{0},taskid:{1}",orderId,processId));
            Response response= jbpm4BizTabServiceFeign.saveJbpm4BizTab(jbpm4BizTabParam);
            if(Response.isSucess(response)){
                log.info(MessageFormat.format("4.startflow.fuyi.sucess,jbpm4BizTabParam:{0}",jbpm4BizTabParam.toString()));
                return true;
            }
            else{
